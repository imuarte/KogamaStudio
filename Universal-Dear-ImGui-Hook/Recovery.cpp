...
namespace Recovery {
    ...
    static void RenderCorruptionPopup()
    {
        ...
        bool open = true;
        if (ImGui::Begin((std::string(TR(u8"Corrupted World Data Detected")) + u8"###CorruptionWarning").c_str(), &open, flags))
        {
            int count = (int)problems.size();
            if (count > 0)
                ImGui::Text(TR(u8"Found %d problem(s)"), count);
            ...
            if (currentPage == 0)
            {
                ImGui::TextWrapped(TR(u8"One or more world objects could not be loaded due to corrupted data. The game has continued loading with the damaged content skipped. Your world may appear incomplete or behave unexpectedly."));

                ImGui::Spacing();
                ImGui::Separator();
                ImGui::Spacing();

                ImGui::TextUnformatted(TR(u8"You can try to recover your world manually:"));
                ImGui::Spacing();

                ImGui::Bullet(); ImGui::SameLine(); ImGui::TextWrapped(TR(u8"Camera unresponsive? Enable Freecam in the View tab."));
                ImGui::Bullet(); ImGui::SameLine(); ImGui::TextWrapped(TR(u8"Use the Explorer panel to browse and select objects."));
                ImGui::Bullet(); ImGui::SameLine(); ImGui::TextWrapped(TR(u8"Use the Properties panel to reposition or delete problematic objects."));
                ImGui::Bullet(); ImGui::SameLine(); ImGui::TextWrapped(TR(u8"Use the Recovery panel for advanced tools (e.g. inventory item removal)."));

                ImGui::Spacing();
                ImGui::Separator();
                ImGui::Spacing();

                if (ImGui::Button(TR(u8"I understand"))) corruptionDismissed = true;
                if (count > 0)
                {
                    ImGui::SameLine();
                    if (ImGui::Button(TR(u8"Next >"))) currentPage = 1;
                }
            }
            else
            {
                ...
                ImGui::Separator();
                if (ImGui::Button(TR(u8"< Back"))) currentPage = 0;
                ImGui::SameLine();
                if (ImGui::Button(TR(u8"Close"))) corruptionDismissed = true;
            }
        }
        ImGui::End();
    ...
    void Render() {
        ...
        if (recoveryOpen) {
            ...
            if (ImGui::Begin((std::string(TR(u8"Recovery")) + u8"###Recovery").c_str(), nullptr))
            {
                if (ImGui::BeginTabBar(u8"Tabs"))
                {
                    if (ImGui::BeginTabItem(TR(u8"Tools")))
                    {
                    ...
                    if (ImGui::BeginTabItem(TR(u8"Fixes")))
                    {
                        ImGui::EndTabItem();
                    }
                    if (ImGui::BeginTabItem(TR(u8"Danger Zone")))
                    {
                        ...
                        if (cats.empty())
                        {
                            ImGui::TextDisabled(TR(u8"No inventory loaded. Refresh inventory first."));
                        }
                        else
                        {
                            ...
                            if (!confirmRemoveCat)
                            {
                                ...
                                if (ImGui::Button(TR(u8"Remove All Items in Category"), ImVec2(-1, 0))) confirmRemoveCat = true;
                                ImGui::PopStyleColor(3);
                            }
                            else
                            {
                                ImGui::TextColored(ImVec4(1, 0.3f, 0.3f, 1), TR(u8"Are you sure? This cannot be undone!"));
                                ImGui::PushStyleColor(ImGuiCol_Button, ImVec4(0.6f, 0.1f, 0.1f, 1.f));
                                ImGui::PushStyleColor(ImGuiCol_ButtonHovered, ImVec4(0.8f, 0.2f, 0.2f, 1.f));
                                ImGui::PushStyleColor(ImGuiCol_ButtonActive, ImVec4(0.5f, 0.05f, 0.05f, 1.f));
                                if (ImGui::Button(TR(u8"Confirm Remove"), ImVec2(-1, 0))) { SendCommand(("recovery_remove_category|" + selectedCatId).c_str()); confirmRemoveCat = false; }
                                ImGui::PopStyleColor(3);
                                if (ImGui::Button(TR(u8"Cancel"), ImVec2(-1, 0))) confirmRemoveCat = false;
                            }
                        }
                        ImGui::EndTabItem();
                    }
